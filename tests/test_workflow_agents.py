...
def test_norm_graph_accepts_the_execute_tier():
    g = workflows._norm_graph({"nodes": [
        {"id": 1, "type": "agent", "task": "t", "write": "execute"},
        {"id": 2, "type": "delegate", "text": "t", "write": "execute"},
    ], "edges": []})
    assert {n["write"] for n in g["nodes"]} == {"execute"}
