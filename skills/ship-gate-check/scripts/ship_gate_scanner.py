...
CHECKS = {
    ...
    "SEC-13": CheckDef("SEC-13", "No eval() or dangerouslySetInnerHTML without sanitization", Severity.HIGH, "SEC", stack="js"),  # auditor:ignore-line
...
def check_sec13(root, stack):
    ...
    for f in dsi_findings:
        try:
            ...
            unsafe_dsi.append(f)
    all_findings = eval_findings + unsafe_dsi  # auditor:ignore-line
    if all_findings:
        return Result(c, Status.FAIL, "Unsafe eval() or unsanitized dangerouslySetInnerHTML", all_findings)  # auditor:ignore-line
    return Result(c, Status.PASS)
