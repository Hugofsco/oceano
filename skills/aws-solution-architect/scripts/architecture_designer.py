...
CLI added locally (the upstream module ships as import-only, with no
argparse/__main__ despite its SKILL.md documenting a --input CLI).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Recommend an AWS architecture pattern from requirements JSON.")
    parser.add_argument("--input", required=True, help="Path to requirements JSON (application_type, expected_users, "
                         "requests_per_second, budget_monthly_usd, team_size, aws_experience, compliance, data_size_gb)")
    ...
    result["service_checklist"] = designer.generate_service_checklist()
